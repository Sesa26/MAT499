<template>
	<div class="container-fluid">
		<p class="white-text h2 blue darken-3 ">ESTERS</p>
		...
		<mdb-row class="justify-content-center">
			...
			<mdb-col v-else col="5" class="d-flex">
				...
				<span class="big">H<sub class="small">{{hydrogen}}</sub></span>
		...
		<mdb-row class="justify-content-center">
			<div class="d-flex" v-if="carbon > 0">
				...
				<span v-for="i in mids2" :key="i" class="d-flex">
					<horizontal />
					<twoHydrogen  />
				</span>
					<horizontal />
...
export default {
	...
	computed: {
		...
		mids2(){
			return this.carbon2>1? this.carbon2-1:0;
		}
	},
	methods: {
		convertFromName() {
			...
			let sub = this.name;
			sub.toLowerCase();
			sub = sub.replace("anoic ethanoate", "");
		...
		convertFromMoleculaFormula() {
			...
			if (this.carbon == 0) {
				...
				this.name = "methyl methanoate";
			} else if (this.carbon == 2) {
				this.name = "ethyl ethanoate";
			} else if (this.carbon == 3) {
				this.name = "propyl ethanoate";
			} else if (this.carbon == 4) {
				this.name = "butyl ethanoate";
			} else if (this.carbon == 5) {
				this.name = "pentyl ethanoate";
			} else if (this.carbon == 6) {
				this.name = "hexyl ethanoate";
			} else if (this.carbon == 7) {
				this.name = "heptyl ethanoate";
			} else if (this.carbon == 8) {
				this.name = "octyl ethanoate";
			} else if (this.carbon == 9) {
				this.name = "nonyl ethanoate";
			} else if (this.carbon == 10) {
				this.name = "decyl ethanoate";
			}
		},
		carbonChange() {
